...
/* CHIENLUOCUUDAI
 */
CREATE OR REPLACE PROCEDURE USP_CHIENLUOCUUDAI_INS(
    ITENCL VARCHAR2, IMOTA VARCHAR2, ILDDEXUAT VARCHAR2, IMACL OUT VARCHAR2
) AS BEGIN
    BEGIN SELECT MACL INTO IMACL FROM CHIENLUOCUUDAI
    WHERE MACL = (SELECT MAX(MACL) FROM CHIENLUOCUUDAI);
    EXCEPTION WHEN NO_DATA_FOUND THEN IMACL := NULL; END;
    IMACL := UFN_MAKE_ID('CL', IMACL);

    INSERT INTO CHIENLUOCUUDAI VALUES(IMACL, ITENCL, IMOTA, ILDDEXUAT);
END;
/